...
Seaport是一个新的市场协议，用于安全和有效地购买和销售NFT。
...
Seaport是一个市场协议，用于安全和有效地购买和销售NFTs。每个列表包含一个任意数量的物品，offerer愿意提供（"offer"），以及一个任意数量的物品，这些物品必须与它们各自的receivers一起被接收（"consideration"）。
...
优化的合同是使用IR管道编译的，这可能需要很长的时间来编译。默认情况下，差异测试套件部署了优化和参考合同的预编译版本。预编译可以通过指定特定的 Foundry 配置文件来完成。
